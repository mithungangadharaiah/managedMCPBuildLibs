...
export const listAuditLogs = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.filter)
            params.append('filter', parsed.data.filter);
        if (parsed.data.pageSize)
            params.append('pageSize', parsed.data.pageSize.toString());
        if (parsed.data.nextPageKey)
            params.append('nextPageKey', parsed.data.nextPageKey);
        try {
            const response = await axios.get(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/auditlogs?${params}`, {
                headers: {
...
//# sourceMappingURL=list-audit-logs.js.map