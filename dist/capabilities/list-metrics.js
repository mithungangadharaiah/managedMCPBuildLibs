...
export const listMetrics = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.metricSelector)
            params.append('metricSelector', parsed.data.metricSelector);
        if (parsed.data.text)
            params.append('text', parsed.data.text);
        ...
        try {
            const response = await axios.get(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/metrics?${params}`, {
                headers: {
...
//# sourceMappingURL=list-metrics.js.map