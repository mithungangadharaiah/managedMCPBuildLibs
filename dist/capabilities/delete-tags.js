...
export const deleteTags = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.value)
            params.append('value', parsed.data.value);
        if (parsed.data.deleteAllWithKey !== undefined)
            params.append('deleteAllWithKey', parsed.data.deleteAllWithKey.toString());
        if (parsed.data.from)
            params.append('from', parsed.data.from);
        if (parsed.data.to)
            params.append('to', parsed.data.to);
        try {
            const response = await axios.delete(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/tags?${params}`, {
                headers: {
...
//# sourceMappingURL=delete-tags.js.map