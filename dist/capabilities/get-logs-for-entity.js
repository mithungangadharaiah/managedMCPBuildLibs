...
export const getLogsForEntity = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.sort)
            params.append('sort', parsed.data.sort);
        if (parsed.data.limit)
            params.append('limit', parsed.data.limit.toString());
        if (parsed.data.nextPageKey)
            params.append('nextPageKey', parsed.data.nextPageKey);
        try {
            const response = await axios.get(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/logs/search?${params}`, {
                headers: {
...
//# sourceMappingURL=get-logs-for-entity.js.map