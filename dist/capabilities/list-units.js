...
export const listUnits = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.unitSelector)
            params.append('unitSelector', parsed.data.unitSelector);
        ...
        try {
            const response = await axios.get(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/units?${params}`, {
                headers: {
