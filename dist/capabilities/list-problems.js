...
export const listProblems = {
    ...
    handler: async (args, dtEnv) => {
        ...
        if (parsed.data.from)
            params.append('from', parsed.data.from);
        if (parsed.data.to)
            params.append('to', parsed.data.to);
        if (parsed.data.problemSelector)
            params.append('problemSelector', parsed.data.problemSelector);
        if (parsed.data.entitySelector)
            params.append('entitySelector', parsed.data.entitySelector);
        if (parsed.data.fields)
            params.append('fields', parsed.data.fields);
        if (parsed.data.pageSize)
            params.append('pageSize', parsed.data.pageSize.toString());
        try {
            const response = await axios.get(`${dtEnv.url}/e/${dtEnv.environmentId}/api/v2/problems?${params}`, {
                headers: {
...
//# sourceMappingURL=list-problems.js.map